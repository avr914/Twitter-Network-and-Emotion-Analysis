...
FOLLOWERS_OF_FOLLOWERS_LIMIT = 250
...
def get_follower_ids(centre, max_depth=1, current_depth=0, taboo_list=[]):
    ...
    try:
            ...
            with open(fname, 'w') as outf:
                ...
                while True:
                    try:
                        ...
                        follower_dict[follower.id] = params
                ...
                while True:
                    try:
                        ...
                        friend_dict[friend.id] = params
                        #writer.writerow(params)
                        friend_count += 1
                        if friend_count >= MAX_FRIENDS:
                            print "Reached max no. of friend for '%s'." % friend.screen_name
                            break
                    except tweepy.TweepError, error:
                        # hit rate limit, sleep for 15 minutes
                        print 'Rate limited. Sleeping for 15 minutes.'
                ...
                intersect_set = follower_dict.viewkeys() & friend_dict.viewkeys()
                for ele in intersect_set:
                    writer.writerow(friend_dict[ele])
                
        else:
            with open(fname, 'r') as outf:
                reader = csv.reader(outf,dialect='excel')
                followerids = [row[0] for row in reader]
        ...
        if mark and os.path.exists(fname):
             os.remove(fname)
